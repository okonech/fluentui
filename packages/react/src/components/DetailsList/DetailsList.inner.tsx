...
import type { IGroupedList, IGroupDividerProps, IGroup } from '../../GroupedList';
import type { IListProps } from '../../List';
import { IDetailsListState } from './DetailsList.base';

const getClassNames = classNamesFunction<IDetailsListStyleProps, IDetailsListStyles>();

const DEFAULT_RENDERED_WINDOWS_AHEAD = 2;
const DEFAULT_RENDERED_WINDOWS_BEHIND = 2;
const DEFAULT_INNER_ROLE = 'grid';
 ...
 */
export const DetailsListInner: React.ComponentType<IDetailsListInnerProps> = (
  ...
  const role = props.role ? props.role : DEFAULT_INNER_ROLE;
  ...
  // const oldListProps = React.useRef([isSizing, getKey, version, listProps] as any[]);
  const additionalListProps = React.useMemo((): IListProps => {
    // [isSizing, getKey, version, listProps].forEach((prop,idx) => {
    //   prop !== oldListProps.current[idx] && console.log(idx, prop,oldListProps.current[idx]);
    // });

    // oldListProps.current=[isSizing, getKey, version, listProps];
    return {
  ...
  const rowRole = React.useMemo(() => (role === DEFAULT_INNER_ROLE ? undefined : 'presentation'), [role]);

  const finalOnRenderRow = React.useMemo(
    () => (props.onRenderRow ? composeRenderFunction(props.onRenderRow, onRenderDefaultRow) : onRenderDefaultRow),
    [onRenderDefaultRow, props.onRenderRow],
  );
  ...
  const onRenderListCell = React.useCallback(
    (item: any, itemIndex: number): React.ReactNode => {
      return onRenderCell(0, item, itemIndex);
    },
    [onRenderCell],
  );
  ...
  const getPageSpecification = React.useCallback(() => ({ itemCount: items.length, height: 30 }), [items.length]);
  ...
  const list = React.useMemo(() => {
    [
      ...
      prop !== oldProps.current[idx] && console.log(idx, prop, oldProps.current[idx]);
    });
    ...
    const finalGroupProps = {
      ...groupProps,
      role: role === DEFAULT_INNER_ROLE ? 'rowgroup' : 'presentation',
